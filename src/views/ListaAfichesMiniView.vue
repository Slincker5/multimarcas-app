...
const { posterSmallList, posterSmallRemoveList } = useGetRoutes();
...
const getData = async () => {
  try {
    ...
    let { data } = await axios.get(posterSmallList, {
      headers,
    });
...
const eliminar = async (uuid) => {
  try {
    ...
    let { data } = await axios.delete(posterSmallRemoveList, {
      headers: headers,
      data: info,
    });
      ...
      <div class="flex items-center justify-between p-4 pt-0">
        <div>
          <font-awesome-icon
            :icon="['fas', 'gear']"
            class="mr-1 fa-spin"
          />Llevas {{ total }}/<span class="text-black">90</span>
        </div>
        <router-link
          to="/enviar-afiches-mini"
