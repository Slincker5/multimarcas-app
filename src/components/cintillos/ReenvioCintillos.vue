...
const emit = defineEmits("formReenviarOpen", "formReenviarClose", "enviandoTrue", "enviandoFalse");
const email = ref(
...
async function resend () {
  try {
    ...
    const dataEnvio = {
      receptor: documento.detalles[0].receptor,
      email: email.value,
    ...
    >
      ...
      >
        ...
        <form class="p-4">
          <div class="mb-4">
            ...
            >
              <input
                ...
                v-model="email"
              />
              <button class="px-4 py-3 transition hover:bg-gray-300" @click.prevent="eliminar">
          ...
          <button
            class="w-full px-6 py-2 mb-4 text-sm text-center text-white bg-indigo-800 border border-indigo-900 border-solid rounded-md shadow-lg shadow-black/20" @click.prevent="resend"
          >
            Reenviar Documento
          </button>
        </form>
